...
function ModelComparison() {
    ...
    const runComparison = async () => {
        ...
        // Fire sequential requests to prevent 429 concurrency limits on OpenRouter free tier
        for (const modelId of selectedModels) {
            try {
                ...
                setResults(prev => ({
                    ...prev,
                    [modelId]: { error: err.response?.data?.message || err.message, loading: false, latency: 0, model: modelId }
                }));
            }
            // Small delay between requests to avoid overlapping limits on Free Models
...
function CompareTab({
    ...
    const [showPaidOpenRouter, setShowPaidOpenRouter] = useState(false);

    const filteredModels = (config?.allModels || []).filter(m => {
        if (m.provider === 'openrouter' && !m.isFree && !showPaidOpenRouter) return false;
        if (providerFilter !== 'all' && m.provider !== providerFilter) return false;
        if (tierFilter !== 'all' && m.tier !== tierFilter) return false;
        if (searchQuery) {
            const sq = searchQuery.toLowerCase();
            return m.label.toLowerCase().includes(sq) || (m.id && m.id.toLowerCase().includes(sq));
        }
        return true;
    });

    const isAllFilteredSelected = filteredModels.length > 0 && filteredModels.every(m => selectedModels.includes(m.id));
    const handleSelectAllFiltered = () => {
        if (isAllFilteredSelected) {
            // Deselect all currently filtered models
            const newSelection = selectedModels.filter(id => !filteredModels.map(m => m.id).includes(id));
            setSelectedModels(newSelection);
        } else {
            // Select all currently filtered models
            const newSelection = [...new Set([...selectedModels, ...filteredModels.map(m => m.id)])];
            setSelectedModels(newSelection);
        }
    };
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={S.card}>
                <div style={S.cardHeader}>
                    ...
                    <button style={S.selectAllBtn} onClick={handleSelectAllFiltered}>
                        {isAllFilteredSelected ? '❌ إلغاء تحديد المعروض' : '✅ تحديد المعروض'}
                    </button>
                    <button style={S.selectAllBtn} onClick={selectAll}>
                        {selectedModels.length === Object.keys(config?.allModels || {}).length ? '🗑️ إلغاء الكل' : '✅ تحديد الكل'}
                    </button>
                </div>

                {/* Filters */}
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 15, background: '#f5f5f5', padding: 12, borderRadius: 8, alignItems: 'center' }}>
                    <input 
                        type="text" 
                        placeholder="🔍 ابحث عن اسم الموديل..." 
                        value={searchQuery} 
                        onChange={(e) => setSearchQuery(e.target.value)}
                        style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #ccc', minWidth: 200, fontFamily: 'inherit' }}
                    />
                    <select value={providerFilter} onChange={(e) => setProviderFilter(e.target.value)} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #ccc', fontFamily: 'inherit' }}>
                        <option value="all">🌐 كل المزودين</option>
                        <option value="openrouter">🌌 OpenRouter</option>
                        <option value="google">🔷 Google / Gemini</option>
                        <option value="openai">🟢 OpenAI</option>
                    </select>
                    <select value={tierFilter} onChange={(e) => setTierFilter(e.target.value)} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #ccc', fontFamily: 'inherit' }}>
                        <option value="all">⚡ كل الفئات</option>
                        <option value="fast">خفيف وسريع</option>
                        <option value="lite">متوسط</option>
                        <option value="pro">متقدم (Pro)</option>
                    </select>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: '#666', cursor: 'pointer', marginLeft: 'auto' }}>
                        <input type="checkbox" checked={showPaidOpenRouter} onChange={e => setShowPaidOpenRouter(e.target.checked)} />
                        💸 إظهار النماذج المدفوعة
                    </label>
                </div>
                    ...
                    {filteredModels.map(model => {
                        ...
                        return (
                            <button key={model.id} className="model-chip"
                                ...
                                <div style={{...S.modelChipId, color: !model.isFree && model.provider === 'openrouter' ? '#d32f2f' : '#999'}}>
                                    {model.id} {!model.isFree && model.provider === 'openrouter' ? '(مدفوع)' : ''}
                                </div>
                ...
                {selectedModels.length > 15 && (
                    <div style={{ color: '#e65100', fontSize: 12, marginTop: 8 }}>
                        ⚠️ اختيار أكثر من 15 نموذج في نفس الوقت قد يسبب أخطاء 429 في النماذج المجانية
                    </div>
                )}
            </div>
