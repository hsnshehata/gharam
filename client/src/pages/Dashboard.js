...
function Dashboard({ user }) {
  ...
  const [bookingFormData, setBookingFormData] = useState({
    packageId: '', hennaPackageId: '', photographyPackageId: '', extraServices: [], returnedServices: [], customExtraServices: [],
    hairStraightening: 'no', hairStraighteningPrice: '', hairStraighteningDate: '',
    hairDye: 'no', hairDyePrice: '', hairDyeDate: '',
    clientName: '', clientPhone: '', city: '', notes: '', eventDate: '', hennaDate: '', deposit: '', paymentMethod: 'cash'
  });
  ...
  useEffect(() => {
    const calculateTotal = () => {
      ...
      if (bookingFormData.customExtraServices) {
        bookingFormData.customExtraServices.forEach(srv => {
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const submitData = {
      ...
      customExtraServices: bookingFormData.customExtraServices,
    ...
    <Container className="mt-5">
      ...
      <div className="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between mb-2 mt-4 gap-3">
        ...
        <div className="d-flex flex-wrap gap-2 w-100 w-md-auto">
          <Form.Select 
            size="sm" 
            style={{ width: '200px', flexGrow: 1, maxWidth: '260px' }} 
            value={logFilter}
            onChange={(e) => setLogFilter(e.target.value)}
          >
            <option value="all">📋 عرض الكل</option>
            <optgroup label="📦 الحجوزات والأقساط">
              <option value="__cat_booking">كل عمليات الحجوزات</option>
            ...
            <optgroup label="⚡ الخدمات الفورية">
              <option value="__cat_instant">كل عمليات الخدمات الفورية</option>
            ...
            <optgroup label="🏢 إشراف الصالة">
              <option value="__cat_supervision">كل عمليات الإشراف</option>
              <option value="⚡ تكليف خدمة">تكليف خدمة</option>
              <option value="🔄 سحب تكليف">سحب تكليف</option>
              <option value="🙋 سحب ذاتي">سحب ذاتي (موظف)</option>
            </optgroup>
            <optgroup label="💰 المالية">
              <option value="__cat_finance">كل العمليات المالية</option>
              ...
              <option value="خصم إداري">خصم إداري</option>
              <option value="تعديل خصم إداري">تعديل خصم إداري</option>
              <option value="حذف خصم إداري">حذف خصم إداري</option>
            </optgroup>
            <optgroup label="👤 إدارة الموظفين">
              <option value="__cat_users">كل عمليات الموظفين</option>
              <option value="إنشاء حساب">إنشاء حساب</option>
              <option value="تعديل حساب">تعديل حساب</option>
              <option value="حذف حساب">حذف حساب</option>
              <option value="🔄 إعادة تعيين">إعادة تعيين الرواتب</option>
            </optgroup>
            <optgroup label="🎁 النقاط والعملات">
              <option value="__cat_points">كل عمليات النقاط</option>
              <option value="🎁 إهداء نقاط">إهداء نقاط</option>
              <option value="📉 خصم نقاط">خصم نقاط</option>
              <option value="🪙 تحويل عملات">تحويل عملات</option>
              <option value="💰 استبدال عملات">استبدال عملات</option>
            </optgroup>
                ...
                {operationsData && operationsData.length > 0 ? (
                  operationsData
                    .filter(op => {
                      ...
                      if (logFilter === '__cat_booking') return op.type.includes('حجز') || op.type.includes('قسط');
                      if (logFilter === '__cat_instant') return op.type.includes('خدمة فورية');
                      if (logFilter === '__cat_supervision') return op.type.includes('تكليف') || op.type.includes('سحب');
                      if (logFilter === '__cat_finance') return op.type.includes('مصروف') || op.type.includes('سلفة') || op.type.includes('خصم إداري');
                      if (logFilter === '__cat_users') return op.type.includes('حساب') || op.type.includes('إعادة تعيين');
                      if (logFilter === '__cat_points') return op.type.includes('نقاط') || op.type.includes('عملات');
                      return op.type === logFilter;
                    })
                    .map((op) => {
                      let badgeClass = 'bg-secondary';
                      const t = op.type;
                      if (t.includes('حجز') && !t.includes('حذف')) badgeClass = 'bg-primary';
                      else if (t.includes('قسط') || t.includes('دفعة')) badgeClass = 'bg-info';
                      else if (t.includes('خدمة فورية') && !t.includes('حذف')) badgeClass = 'bg-success';
                      else if (t.includes('تكليف')) badgeClass = 'bg-primary';
                      else if (t.includes('سحب تكليف')) badgeClass = 'bg-warning text-dark';
                      else if (t.includes('سحب ذاتي')) badgeClass = 'bg-info text-dark';
                      else if (t.includes('مصروف') && !t.includes('حذف')) badgeClass = 'bg-danger';
                      else if (t.includes('سلفة') && !t.includes('حذف')) badgeClass = 'bg-warning text-dark';
                      else if (t.includes('خصم إداري')) badgeClass = 'bg-danger';
                      else if (t.includes('إهداء')) badgeClass = 'bg-success';
                      else if (t.includes('خصم نقاط')) badgeClass = 'bg-danger';
                      else if (t.includes('تحويل عملات') || t.includes('استبدال عملات')) badgeClass = 'bg-info';
                      else if (t.includes('حساب') && !t.includes('حذف')) badgeClass = 'bg-primary';
                      else if (t.includes('تسجيل دخول')) badgeClass = 'bg-secondary';
                      else if (t.includes('إعادة تعيين')) badgeClass = 'bg-warning text-dark';
                      else if (t.includes('حذف')) badgeClass = 'bg-dark text-white';
                  ...
                  <tr>
                    <td colSpan="6" className="text-center py-4">لا توجد عمليات مسجلة لهذا اليوم</td>
                  </tr>
                )}
                {operationsData && operationsData.length > 0 && 
            ...
            <Row className="g-3">
              ...
              <Col md={12}>
                <div className="border p-3 rounded mb-3" style={{ borderColor: 'var(--border)' }}>
                  ...
                  {bookingFormData.customExtraServices && bookingFormData.customExtraServices.length > 0 && (
                    <div className="mt-3">
                      <h6 className="text-secondary" style={{ fontSize: '0.9rem' }}>الخدمات الخاصة المضافة:</h6>
                      <div className="d-flex flex-wrap gap-2">
                        {bookingFormData.customExtraServices.map(srv => (
