...
export const printReceiptElement = (element) => {
  ...
  style.textContent = `
    @media print {
      ...
      #receipt-print-container thead th { color: #000 !important; background-color: #fff !important; background: #fff !important; }
    }
  `;
...
const ReceiptPrint = ({ data, type, autoPrint = true }) => {
  const containerRef = React.useRef(null);

  React.useEffect(() => {
    if (autoPrint && data) {
      // تأخير بسيط للسماح للصور والـ QR Code بالتحميل
      const timer = setTimeout(() => {
        if (containerRef.current) {
          printReceiptElement(containerRef.current);
        }
      }, 500);
