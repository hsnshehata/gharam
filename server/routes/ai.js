...
router.get('/media/:id', async (req, res) => {
    try {
        const media = await AIMedia.findById(req.params.id);
        if (!media) return res.status(404).json({ message: 'الصورة غير موجودة' });
        
        // إذا كانت البيانات عبارة عن رابط خارجي (Cloudinary)، قم بتوجيه مباشر
        if (media.data.startsWith('http')) {
            return res.redirect(media.data);
        }

        const buffer = Buffer.from(media.data, 'base64');
        res.setHeader('Content-Type', media.contentType || 'image/png');
        res.setHeader('Cache-Control', 'public, max-age=31536000');
        res.send(buffer);
    } catch (err) {
        console.error('Error fetching AI media:', err.message);
