...
exports.addUser = async (req, res) => {
  ...
  try {
    ...
    await logActivity({
      ...
      entityType: 'User',
      entityId: user._id,
      details: `تم إنشاء حساب جديد: "${username}" بدور "${role === 'admin' ? 'مدير' : role === 'supervisor' ? 'مشرف' : role === 'hallSupervisor' ? 'مشرف صالة' : 'موظف'}"`,
...
exports.updateUser = async (req, res) => {
  ...
  try {
    ...
    // تسجيل التعديلات
    const changes = [];
    if (username !== oldUser.username) changes.push(`تغيير الاسم من "${oldUser.username}" إلى "${username}"`);
    if (role !== oldUser.role) changes.push(`تغيير الدور من "${oldUser.role}" إلى "${role}"`);
    if (monthlySalary !== undefined && Number(monthlySalary) !== oldUser.monthlySalary) changes.push(`تغيير الراتب من ${oldUser.monthlySalary || 0} إلى ${monthlySalary}`);
    if (password) changes.push('تغيير كلمة المرور');
    if (phone !== oldUser.phone) changes.push(`تغيير الهاتف`);

    await logActivity({
      action: 'UPDATE',
      entityType: 'User',
      entityId: user._id,
      details: `تم تعديل بيانات "${user.username}": ${changes.length > 0 ? changes.join(' | ') : 'لا تغييرات جوهرية'}`,
      performedBy: req.user.id,
      targetUser: user._id
    });
...
exports.resetSalaries = async (req, res) => {
  ...
  try {
    ...
    await logActivity({
      action: 'RESET',
      entityType: 'Salary',
      details: 'تم إعادة شحن رواتب جميع الموظفين يدوياً',
      performedBy: req.user.id
    });
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    ...
    await logActivity({
      ...
      entityType: 'User',
      entityId: user._id,
      details: `تم حذف حساب "${user.username}" (${user.role === 'admin' ? 'مدير' : user.role === 'supervisor' ? 'مشرف' : user.role === 'hallSupervisor' ? 'مشرف صالة' : 'موظف'})`,
      performedBy: req.user.id,
      targetUser: user._id
    });
...
exports.giftPoints = async (req, res) => {
  ...
  try {
    ...
    await logActivity({
      ...
      entityType: 'Points',
      entityId: user._id,
      details: `تم إهداء ${pointsValue} نقطة للموظف "${user.username}" بواسطة "${giftedByName}" - ${note || 'هدية نقاط'}`,
      amount: pointsValue,
      performedBy: req.user.id,
      targetUser: userId
    });
...
exports.giftPointsBulk = async (req, res) => {
  ...
  try {
    ...
    await logActivity({
      action: 'GIFT',
      entityType: 'Points',
      details: `تم إهداء ${pointsValue} نقطة جماعية لـ ${sent} موظف بواسطة "${giftedByName}" - ${note || 'هدية نقاط جماعية'}`,
      amount: pointsValue,
      performedBy: req.user.id
    });
...
exports.openGift = async (req, res) => {
  try {
    ...
    await logActivity({
      ...
      entityType: 'Points',
      entityId: user._id,
      details: `قام الموظف "${user.username}" بفتح هدية نقاط بقيمة ${amount} نقطة - ${target.note || 'هدية'}`,
      amount: amount,
      performedBy: req.user.id,
      targetUser: user._id
    });
...
exports.deductPoints = async (req, res) => {
  ...
  try {
    ...
    await logActivity({
      ...
      entityType: 'Points',
      entityId: user._id,
      details: `تم خصم ${deduction} نقطة من الموظف "${user.username}" بواسطة "${giftedByName}" - السبب: ${reason || 'خصم نقاط إداري'}`,
      amount: deduction,
      performedBy: req.user.id,
      targetUser: userId
    });
...
exports.convertPointsToCoins = async (req, res) => {
  try {
    ...
    await logActivity({
      ...
      entityType: 'Coins',
      entityId: user._id,
      details: `قام الموظف "${user.username}" بتحويل ${mintedCoins.length} عملة جديدة (المستوى ${coinLevel} - قيمة العملة ${coinValue} ج.م)`,
      amount: mintedCoins.length * coinValue,
      performedBy: req.user.id,
      targetUser: user._id
    });
...
exports.redeemCoins = async (req, res) => {
  try {
    ...
    await logActivity({
      ...
      entityType: 'Coins',
      entityId: user._id,
      details: `قام الموظف "${user.username}" باستبدال ${coinsToRedeem.length} عملة بقيمة ${totalRedeemedValue} ج.م وإضافتها للمرتب`,
      amount: totalRedeemedValue,
      performedBy: req.user.id,
      targetUser: user._id
    });
