...
exports.addBooking = async (req, res) => {
  const {
    packageId, hennaPackageId, photographyPackageId, returnedServices, extraServices, customExtraServices,
  ...
  try {
    ...
    let customExtraPrice = 0;
    if (customExtraServices && customExtraServices.length > 0) {
      customExtraPrice = customExtraServices.reduce((sum, srv) => sum + (Number(srv.price) || 0), 0);
    }
    ...
    let formattedPackageServices = packageServices.map(srv => ({
      _id: srv._id,
      name: srv.name,
      price: srv.price,
      isCustom: false,
    ...
    if (customExtraServices && customExtraServices.length > 0) {
      const formattedCustomServices = customExtraServices.map(srv => ({
        _id: srv._id,
        name: srv.name,
      ...
      formattedPackageServices = [...formattedPackageServices, ...formattedCustomServices];
    }
    ...
    const booking = new Booking({
      ...
      returnedServices, extraServices, customExtraServices, packageServices: formattedPackageServices,
      hairStraightening, hairStraighteningPrice, hairStraighteningDate,
      hairDye, hairDyePrice, hairDyeDate,
      clientName, clientPhone, city, notes, eventDate, hennaDate, deposit,
...
exports.updateBooking = async (req, res) => {
  ...
  try {
    ...
    if (oldBooking.notes !== notes) changes.notes = notes;
  ...
  try {
    ...
    await logActivity({
      ...
      details: isSelfExecute
        ? `قام الموظف "${user.username}" بسحب وتنفيذ خدمة "${serviceName}" بنفسه - وصل ${booking.receiptNumber || '-'} - عميل: ${booking.clientName}`
        : `تكليف خدمة "${serviceName}" للموظف "${user.username}" بواسطة "${performerName}" - وصل ${booking.receiptNumber || '-'} - عميل: ${booking.clientName}`,
      amount: points,
      performedBy: req.user.id,
...
exports.resetService = async (req, res) => {
  ...
  try {
    ...
    let points = 0;
    let resetServiceName = 'غير معروف';
    ...
    if (serviceId === 'hairStraightening' || serviceId === 'hairDye' || serviceId === 'photography') {
      ...
      points = service.price * 0.15;
      resetServiceName = service.name || 'خدمة باكدج';
      service.executed = false;
      service.executedBy = null;
      service.executedAt = null;
    }

    // جلب اسم الموظف القديم للتسجيل
    ...
    await logActivity({
      ...
      entityType: 'Service',
      entityId: booking._id,
      details: `سحب التكليف من الموظف "${oldEmployeeName}" لخدمة "${resetServiceName}" بواسطة "${resetPerformerName}" - وصل ${booking.receiptNumber || '-'} - عميل: ${booking.clientName}`,
      amount: points,
      performedBy: req.user.id,
