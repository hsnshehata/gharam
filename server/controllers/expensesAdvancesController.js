  ...
  try {
    ...
    if (type === 'expense') {
      ...
      await logActivity({
        ...
        details: `خصم إداري ${amount} ج.م من الموظف "${populatedDeduction.userId?.username || ''}" - السبب: ${details}`,
        amount,
        performedBy: creatorId,
        targetUser: userId
      });
  ...
  try {
    ...
    if (type === 'expense') {
      ...
      await logActivity({
        ...
        entityType: 'Deduction',
        entityId: deduction._id,
        details: `تعديل خصم إداري للموظف "${deduction.userId?.username || ''}" - المبلغ: ${amount} ج.م - السبب: ${details}`,
        amount,
        performedBy: creatorId,
        targetUser: userId
      });
  ...
  try {
    ...
    if (type === 'expense') {
      ...
      await logActivity({
        ...
        entityType: 'Deduction',
        entityId: deduction._id,
        details: `حذف خصم إداري للموظف "${user?.username || ''}" - المبلغ: ${deduction.amount} ج.م - السبب: ${deduction.reason || ''}`,
        amount: deduction.amount,
        performedBy: req.user.id,
        targetUser: deduction.userId
      });
